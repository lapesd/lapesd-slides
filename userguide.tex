...
\begin{frame}[fragile]{Origem e Uso}
  ...
  \item Esse template funciona como uma classe \LaTeX
    \begin{itemize}
    \item O .cls é standalone: \textbf{não depende} de arquivos externos
    \item Obtenha atualizações no \href{https://github.com/lapesd/lapesd-slides/}{GitHub}
    \item \href{http://fixme.org/todo}{Template no Overleaf} 
      \begin{itemize}
...
\end{minted}
  ...
  \item Atalho para incluir figuras
    ...
    \item Havera espaçamento de \texttt{.5em} (meia linha) antes e depois
...
\begin{frame}[fragile]{\texttt{\textbackslash{}\textbf{t}cite*}}
  ...
  \end{center}
  \begin{itemize}
  \item Equivalentes às suas contrapartes sem o \textbf{\texttt{t}}
  \item Não são definios se o \mla|\cite| correspondente não existir
...
\begin{frame}{Instruções para animações com svg2pdf}
  ...
  \item Crie um grupo no SVG para cada frame
    ...
    \item Todos os frames (grupos) devem estar epilhados no eixo $y$, isto é deve existir uma reta paralela ao eixo $y$ que contenha todos os top-left dos bounding box de todos os grupos
    \item Não devem haver elementos no SVG fora do bounding box definido pelo top-left de frame com maior valor de $y$ e o bottom-right de frame com o menor valor de $y$   \end{itemize}
  \item Nomeie grupos no XML do SVG
  \item No Makefile, adicione um \emph{target} e uma entrada na variável \texttt{frames} para cada grupo
